...
class Googledrive {
    ...
    public function smallfileupload($path, $tmpfile) {
        ...
        $parent = $this->list_path($path);
        if (!isset($parent['id'])) {
            return output('Parent folder not found', 404);
        }
        $result = $this->createFile_c($parent['id'], $filename, $content);
        $res = $this->files_format(json_decode($result['body'], true));
        if (isset($res['url'])) {
            $res['url'] = $_SERVER['host'] . path_format($_SERVER['base_disk_path'] . '/' . $path . '/' . $filename);
        }
    ...
    public function bigfileupload($path) {
        ...
        if (!isset($parent['id'])) {
            ...
            $parent['id'] = json_decode($res['body'], true)['id'];
        }

        $url = 'https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable';
        $tmp['name'] = $filename;
        $tmp['parents'][0] = $parent['id'];
